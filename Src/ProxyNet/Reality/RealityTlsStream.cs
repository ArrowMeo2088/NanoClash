...
namespace Clash.ProxyNet.Reality;
...
internal sealed class RealityTlsStream : Stream
{
    ...
    /// <summary>Underlying TCP (or transport) under REALITY — used for Vision Direct splice.</summary>
    internal Stream Transport => _transport;

    /// <summary>Copies out decrypted app-data still held from the last record.</summary>
    internal byte[] StealPending()
    {
        if (_pending.IsEmpty)
            return [];
    ...
    /// <summary>Bytes buffered from TCP that are not a complete TLS record yet (post-Direct raw).</summary>
    internal byte[] StealInboundLeftover() => _records.StealInboundLeftover();

    /// <summary>
    /// Vision Direct: subsequent reads go to raw TCP. This stream keeps writing REALITY records
    /// on the same socket until disposed, but no longer closes the transport.
    /// </summary>
    internal Stream DetachTransport()
    {
    ...
    public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return await ReadFromRecordsAsync(buffer, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _readGate.Release();
        ...
        ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await _records.WriteApplicationDataAsync(buffer, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _writeGate.Release();
