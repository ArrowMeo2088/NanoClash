...
internal sealed class TlsRecordStream(Stream transport) : IDisposable
{
    ...
    /// <summary>
    /// After Vision <c>Direct</c>, the peer stops speaking TLS records on this TCP socket.
    /// Any bytes already pulled into the inbound buffer but not yet a full record are the start
    /// of that raw stream (mihomo/Xray <c>rawInput</c>).
    /// </summary>
    public byte[] StealInboundLeftover()
    {
        int n = _end - _start;
        if (n <= 0)
            return [];

        var copy = _inbound.AsSpan(_start, n).ToArray();
        _start = _end = 0;
        return copy;
    }
}
